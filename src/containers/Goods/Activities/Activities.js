...
class Activities extends React.Component{
		...
		const columns = [
			...
			},
			{
				...
				render: (text,record) =>
					<div>
						{
							window.hasPermission("product_activity_book_set_sale_scope") && <span
								style={{'color':'#4F9863','cursor':'pointer'}}
	...
	// 商品详情
	jump = (record) =>{
		this.props.history.push({pathname:"/goods/goodDetails",state:{id:record.product_id,path:'/goods/activities', current: this.child.current.state.current}})
	};
